...
#include <memory> // 包含std::shared_ptr的头文件
...
void ImageProcessor::stereoCallback(
  ...
  cv::Mat bgr_image = cv_bridge::toCvShare(cam0_img, sensor_msgs::image_encodings::BGR8)->image;
  cv::Mat gray_image;
  cv::cvtColor(bgr_image, gray_image, cv::COLOR_BGR2GRAY);
  sensor_msgs::ImagePtr new_msg0 = cv_bridge::CvImage(std_msgs::Header(), "mono8", gray_image).toImageMsg();
  new_msg0->header = cam0_img->header; // 复制原始消息的头部信息

  cv::Mat bgr_image1 = cv_bridge::toCvShare(cam1_img, sensor_msgs::image_encodings::BGR8)->image;
  cv::Mat gray_image1;
  cv::cvtColor(bgr_image1, gray_image1, cv::COLOR_BGR2GRAY);
  sensor_msgs::ImagePtr new_msg1 = cv_bridge::CvImage(std_msgs::Header(), "mono8", gray_image1).toImageMsg();
  new_msg1->header = cam1_img->header; // 复制原始消息的头部信息
  // Get the current image.
  cam0_curr_img_ptr = cv_bridge::toCvShare(new_msg0,
      sensor_msgs::image_encodings::MONO8);
  cam1_curr_img_ptr = cv_bridge::toCvShare(new_msg1,
      sensor_msgs::image_encodings::MONO8);
  ...
  // cv::Mat hygray;
  // cv::cvtColor(cam0_curr_img_ptr->image, hygray, cv::COLOR_BGR2GRAY);
  // hygray.convertTo(hygray, CV_8U);
  
  // sensor_msgs::Image imageMsg;
  // imageMsg.header.stamp = cam0_img->header.stamp;
  // imageMsg.header.frame_id = cam0_img->header.frame_id;
  // imageMsg.encoding = sensor_msgs::image_encodings::MONO8;
  // imageMsg.width = hygray.cols;
  // imageMsg.height = hygray.rows;
  // imageMsg.step = hygray.cols * hygray.channels(); // 假设图像是单通道的灰度图像
  // imageMsg.data = std::vector<uint8_t>(hygray.data, hygray.data + hygray.total() * hygray.elemSize());


  // sensor_msgs::ImageConstPtr imagePtr = imageMsg.ConstPtr();

  // std::shared_ptr<sensor_msgs::Image> imagePtr = std::make_shared<sensor_msgs::Image>(imageMsg); 

  // cam0_curr_img_ptr = cv_bridge::toCvShare(imagePtr,
                                          //  sensor_msgs::image_encodings::MONO8); 
  // cam0_curr_img_ptr = cv_bridge::toCvShare(hygray, "mono8");
  // cam0_curr_img_ptr =  cv_bridge::toCvShare(imageMsg, sensor_msgs::image_encodings::MONO8);

  // cv::Mat hygray1;
  // cv::cvtColor(cam1_curr_img_ptr->image, hygray1, cv::COLOR_BGR2GRAY);
  // cam1_curr_img_ptr = cv_bridge::CvImageConstPtr(new cv::Mat(hygray1), sensor_msgs::image_encodings::MONO8);
...
void ImageProcessor::createImagePyramids() {
  ...
  // cv::cvtColor(curr_cam0_img, cam0_curr_img_ptr->image, cv::COLOR_BGR2GRAY);
  ...
  // cv::cvtColor(curr_cam1_img, cam1_curr_img_ptr->image, cv::COLOR_BGR2GRAY);
  buildOpticalFlowPyramid(
...
void ImageProcessor::initializeFirstFrame() {
  ...
  // cv::Mat img;
  // cv::cvtColor(img, cam0_curr_img_ptr->image, cv::COLOR_BGR2GRAY);
...
void ImageProcessor::addNewFeatures() {
  ...
  // cv::Mat curr_img;
  // cv::cvtColor(curr_img, cam0_curr_img_ptr->image, cv::COLOR_BGR2GRAY); 
